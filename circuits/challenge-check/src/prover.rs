//! Off-chain prover pipeline for the challenge-check circuit.
...
use crate::circuit::ChallengeCheckCircuit;

pub struct ChTestVector {
    pub vk_bytes:    Vec<u8>,
...
pub fn generate_ch_test_vector(k: u32, seed: [u8; 32]) -> Result<ChTestVector, anyhow::Error> {
    let mut rng = StdRng::from_seed(seed);

    let params: ParamsKZG<Bn256> = ParamsKZG::<Bn256>::setup(k, &mut rng);

    let circuit = ChallengeCheckCircuit;
...

    {
        ...
        ).map_err(|e| anyhow::anyhow!("halo2 self-verify (challenge, KeccakBe): {e:?}"))?;
        eprintln!("       ✓ halo2 self-verify (challenge, KeccakBe) passed");
    }

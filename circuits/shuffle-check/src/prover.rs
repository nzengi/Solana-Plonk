//! Off-chain prover pipeline for the shuffle-check circuit. No instance
//! columns; the shuffle argument is the only constraint.
...
use crate::circuit::{ShuffleCheckCircuit, SH_LEN};

pub struct ShTestVector {
    pub vk_bytes:    Vec<u8>,
...
pub fn generate_sh_test_vector(k: u32, seed: [u8; 32]) -> Result<ShTestVector, anyhow::Error> {
    let mut rng = StdRng::from_seed(seed);

    let params: ParamsKZG<Bn256> = ParamsKZG::<Bn256>::setup(k, &mut rng);

    // Witness: same multiset across the two columns.
    let input:    [Fr; SH_LEN] = [Fr::from(1u64), Fr::from(2u64), Fr::from(3u64), Fr::from(4u64)];
    let shuffled: [Fr; SH_LEN] = [Fr::from(3u64), Fr::from(1u64), Fr::from(4u64), Fr::from(2u64)];
    let circuit = ShuffleCheckCircuit { input, shuffled };
...

    {
        ...
        ).map_err(|e| anyhow::anyhow!("halo2 self-verify (shuffle, KeccakBe): {e:?}"))?;
        eprintln!("       ✓ halo2 self-verify (shuffle, KeccakBe) passed");
    }

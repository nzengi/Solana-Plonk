//! End-to-end smoke test for the shuffle-check circuit through
//! `halo2-solana-verifier`.

use shuffle_check_circuit::generate_sh_test_vector;
...
fn main() -> Result<(), anyhow::Error> {
    ...
    eprintln!("[1/4] generating KZG params (k={k}) + shuffle witness…");
    let v = generate_sh_test_vector(k, seed)?;
    ...
    if shadow_audit && matches!(result, Ok(true)) {
        shuffle_check_circuit::shadow::audit(&v.params, &v)?;
    }

    match result {
        Ok(true)  => { eprintln!("✓ shuffle proof verified end-to-end"); Ok(()) }
...
fn write_golden_vector(
    v: &shuffle_check_circuit::prover::ShTestVector,
    public_inputs: &[[u8; 32]],
) -> Result<(), anyhow::Error> {
    use std::{fs, io::Write};
    let path = PathBuf::from("circuits/shuffle-check/tests/golden_v2_sh.bin");
    ...
    f.write_all(&buf)?;
    eprintln!("[+] wrote shuffle golden vector → {} ({} B)", path.display(), buf.len());
    Ok(())
}

//! End-to-end smoke test for the range-check (Plookup) circuit through
...
fn main() -> Result<(), anyhow::Error> {
    let seed = [42u8; 32];
    // k=6 → 64 rows. 16-entry table + 8 values + ~5 blinding factors ≈ 30 rows used.
    let k = 6;
    let write_golden = std::env::args().any(|a| a == "--write-golden");
    let shadow_audit = std::env::args().any(|a| a == "--shadow-audit");

    eprintln!("[1/4] generating KZG params (k={k}) + range-check witness…");
    let v = generate_rc_test_vector(k, seed)?;
    ...
    if shadow_audit && matches!(result, Ok(true)) {
        range_check_circuit::shadow::audit(&v.params, &v)?;
    }

    match result {
        Ok(true)  => { eprintln!("✓ range-check proof verified end-to-end"); Ok(()) }
...
fn write_golden_vector(
    v: &range_check_circuit::prover::RcTestVector,
    public_inputs: &[[u8; 32]],
) -> Result<(), anyhow::Error> {
    use std::{fs, io::Write};
    let path = PathBuf::from("circuits/range-check/tests/golden_v2_rc.bin");
    if let Some(p) = path.parent() {
        fs::create_dir_all(p)?;
    }

    // Same magic-prefixed format as Fibonacci's golden_v15_fib.bin
    // (with public-inputs section, which here is zero-length).
    ...
    f.write_all(&buf)?;
    eprintln!("[+] wrote range-check golden vector → {} ({} B)", path.display(), buf.len());
    Ok(())
}

//! Mollusk CU benchmark for the shuffle-check circuit (v2.0).
//!
//! Loads `circuits/shuffle-check/tests/golden_v2_sh.bin`, parses the
//! GLDN0002 layout, repackages into the program's GLDN0001 instruction-data
//! format, runs under Mollusk. First BPF-VM-side test exercising the v2.0
//! shuffle verifier (1 G1 + 2 Fr per shuffle, 2 SHPLONK queries).
...
fn load_sh_golden() -> (Vec<u8>, Vec<u8>, [u8; 64], [u8; 128], [u8; 128], Vec<[u8; 32]>) {
    let mut path = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    path.push("../../circuits/shuffle-check/tests/golden_v2_sh.bin");
    let buf = std::fs::read(&path).unwrap_or_else(|_| panic!(
        "missing {path:?} — run \
         `cargo run -p shuffle-check-circuit --bin gen-sh-proof -- --write-golden` first"
    ));
    let mut cur = 0;
    assert_eq!(&buf[cur..cur + 8], b"GLDN0002", "bad shuffle golden magic");
    cur += 8;
    ...
    assert_eq!(cur, buf.len(), "trailing bytes in shuffle golden file");
...
#[test]
fn shuffle_verify_inside_bpf_vm() {
    let (vk, proof, g1_one, g2_one, g2_tau, pis) = load_sh_golden();
    ...
    eprintln!("[sh] program_result        = {:?}", result.program_result);
    eprintln!("[sh] compute_units_consumed = {}", result.compute_units_consumed);

    match result.program_result {
        ProgramResult::Success => eprintln!("[sh] ✓ shuffle verifier passed inside BPF VM"),
        other => panic!("shuffle BPF run failed: {other:?}"),
    }
}

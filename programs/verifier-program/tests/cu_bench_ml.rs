//! Mollusk CU benchmark for the multi-lookup (2 Plookup args) circuit.
//!
//! Loads `circuits/multi-lookup-check/tests/golden_v2_ml.bin`, parses the
//! GLDN0002 layout, repackages into the program's GLDN0001 instruction-data
//! format, runs under Mollusk. Critical regression test for the v2.0
//! lookup loop iterating `i ∈ {0, 1}` (range-check only iterates `i = 0`).
...
fn load_ml_golden() -> (Vec<u8>, Vec<u8>, [u8; 64], [u8; 128], [u8; 128], Vec<[u8; 32]>) {
    let mut path = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    path.push("../../circuits/multi-lookup-check/tests/golden_v2_ml.bin");
    let buf = std::fs::read(&path).unwrap_or_else(|_| panic!(
        "missing {path:?} — run \
         `cargo run -p multi-lookup-check-circuit --bin gen-ml-proof -- --write-golden` first"
    ));
    let mut cur = 0;
    assert_eq!(&buf[cur..cur + 8], b"GLDN0002", "bad multi-lookup golden magic");
    cur += 8;
    ...
    assert_eq!(cur, buf.len(), "trailing bytes in multi-lookup golden file");
...
#[test]
fn multi_lookup_verify_inside_bpf_vm() {
    let (vk, proof, g1_one, g2_one, g2_tau, pis) = load_ml_golden();
    ...
    eprintln!("[ml] program_result        = {:?}", result.program_result);
    eprintln!("[ml] compute_units_consumed = {}", result.compute_units_consumed);

    match result.program_result {
        ProgramResult::Success => eprintln!("[ml] ✓ multi-lookup verifier passed inside BPF VM"),
        other => panic!("multi-lookup BPF run failed: {other:?}"),
    }
}

//! Mollusk CU benchmark for the range-check (Plookup) circuit (v2.0).
//!
//! Loads `circuits/range-check/tests/golden_v2_rc.bin`, parses the
//! GLDN0002 layout, repackages into the program's GLDN0001 instruction-data
//! format, runs under Mollusk. This is the first BPF-VM-side test exercising
//! the v2.0 lookup verifier (3 G1 + 5 Fr per lookup, 5 SHPLONK queries).
...
fn load_rc_golden() -> (Vec<u8>, Vec<u8>, [u8; 64], [u8; 128], [u8; 128], Vec<[u8; 32]>) {
    let mut path = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    path.push("../../circuits/range-check/tests/golden_v2_rc.bin");
    let buf = std::fs::read(&path).unwrap_or_else(|_| panic!(
        "missing {path:?} — run \
         `cargo run -p range-check-circuit --bin gen-rc-proof -- --write-golden` first"
    ));
    let mut cur = 0;
    assert_eq!(&buf[cur..cur + 8], b"GLDN0002", "bad range-check golden magic");
    cur += 8;
    ...
    assert_eq!(cur, buf.len(), "trailing bytes in range-check golden file");
...
#[test]
fn range_check_verify_inside_bpf_vm() {
    let (vk, proof, g1_one, g2_one, g2_tau, pis) = load_rc_golden();
    ...
    eprintln!("[rc] program_result        = {:?}", result.program_result);
    eprintln!("[rc] compute_units_consumed = {}", result.compute_units_consumed);

    match result.program_result {
        ProgramResult::Success => eprintln!("[rc] ✓ range-check verifier passed inside BPF VM"),
        other => panic!("range-check BPF run failed: {other:?}"),
    }
}
